...
def get_addr_tags(offset,end_of_table_pagination):
        ...
        try:
            url = 'https://www.blockchain.com/btc/tags?filter=2'
...
end_of_table_pagination = 2500   #3650
offset = 0
results = {}
for offset in range(offset, end_of_table_pagination, 50):
        df =get_addr_tags(offset,end_of_table_pagination)
        with open('bitcoin_talk_profiles/'+str(offset) + 'to' + str(end_of_table_pagination) + '.csv', 'a', encoding="utf-8") as f:
            if offset == 0:
